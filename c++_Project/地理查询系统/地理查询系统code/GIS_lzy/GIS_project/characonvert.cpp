...
//判断一个字符串是否为utf8编码
///////////////////////////////
int Characonvert::is_utf8_string(char* utf)
{
...
bool Characonvert::safeAddToOutbuf(char* outbuf,int &iOutbuf,char* pinyinValue,int iPinyinValue)
{
...
void Characonvert::pinyin_utf8(char* inbuf,char* outbuf, bool m_blnSimaple,bool polyphone_support,bool m_blnFirstBig,bool m_blnAllBiG,bool m_LetterEnd, bool m_unknowSkip,bool m_filterPunc)
{
    ...
    for (int i=0;i<inbuf_len;i++)
    {
        if ( (unsigned char)inbuf[i] < 0x80 )  {  //若是英文或数字
            ...
            if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],1)) return;
            ...
            if(!m_unknowSkip){
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],2)) return;
            }
            i++;
...
//                strcpy_s(py_tmp,HZ2PY_MAX_PINYIN_SIZE, this->_pinyin_table_[uni - 19968]);
...
//                    strcpy_s(py_tmp2,HZ2PY_MAX_PINYIN_SIZE, py_tmp);
                ...
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,py_tmp2,strlen(py_tmp2)))    return;
                if(m_LetterEnd) if(!this->safeAddToOutbuf(outbuf,iOutbuf,&sep,1)) return;
                i=i+2;
            }else if( !m_unknowSkip){
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],3)) return;
                i=i+2;
            }
            ...
            if( !m_unknowSkip){
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],4)) return;
            }
            i=i+3;
            ...
            if( !m_unknowSkip){
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],5)) return;
            }
            i=i+4;
            ...
            if( !m_unknowSkip){
                if(!this->safeAddToOutbuf(outbuf,iOutbuf,&inbuf[i],6)) return;
            }
            i=i+5;
        }else{
            if ( !m_unknowSkip)
