function varargout = equalpolycon_pole_arc(varargin)
...
%x01=1.9499079012403*10^(-5)*phi(indx2).^3+4.79161212198753*phi(indx2);
    ...
    %x01为x0的原式，但与mu相关
x0=63.41514*phi(indx2)+0.9404646*(phi(indx2).^3);%课本上中央经线上x0坐标

    %极坐标
    
rho=(yn.^2+(xn-x01).^2)./(2*(xn-x01));  %与x01做差，x01为
epn=asin(yn./rho);
...
%       N .* cot(phi(indx2)) .* (1-cos(E));
  x(indx2)=rho.*sin(E);
 y(indx2)=x0+rho-rho.*cos(E);     %以x0为中央经线x坐标
end
