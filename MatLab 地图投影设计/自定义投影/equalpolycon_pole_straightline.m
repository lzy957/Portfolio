function varargout = equalpolycon_pole_straightline(varargin)
...
if ~isempty(indx2)
    ...
    %论文中的中央经线x0值
x01=1.9499079012403*10^(-5)*phi(indx2).^3+4.79161212198753*phi(indx2)*radius.*mu(indx2);
...
%     x(indx2) = N .* cot(phi(indx2)) .* sin(E);
     y(indx2) = xn+radius*mu(indx2) + ...
       N .* cot(phi(indx2)) .* (1-cos(E));  %用原多圆锥投影的y值
  x(indx2)=rho.*sin(E);
% y(indx2)=xn+rho-rho.*cos(E);
end
