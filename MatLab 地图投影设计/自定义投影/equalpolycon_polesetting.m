function varargout = equalpolycon_polesetting(varargin)
...
phi(indx3)=phi(indx3)/pi*180;
lambda(indx3)=lambda(indx3)/pi*180;
dc=90;
...
x0=1.9499079012403*10^(-5)*phi(indx2).^3+4.79161212198753*phi(indx2);
%     x01=(phi(indx2)*0.9953537)+0.01476138*(phi(indx2)).^3*radius.*mu(indx2);
x01=63.41514*(phi(indx2)/180*pi)+0.9404646*((phi(indx2)/180*pi).^3);

    %极坐标
    
rho=(yn.^2+(xn-x0).^2)./(2*(xn-x0));
epn=asin(yn./rho);
E=epn/180*1.1.*(1-0.0005050505*abs(lambda(indx2))).*...
    (lambda(indx2));
