function varargout = equalpolycon_lati0(varargin)
...
x01=63.41514*phi(indx2)+0.9404646*(phi(indx2).^3);

    %极坐标
    
rho=(yn.^2+(xn-x0).^2)./(2*(xn-x0));    %与x0做差
epn=asin(yn./rho);
...
%       N .* cot(phi(indx2)) .* (1-cos(E));    MATLAB原多圆锥投影x、y公式
  x(indx2)=rho.*sin(E);
 y(indx2)=x0+rho-rho.*cos(E);       %用x0做为中央经线上的x
end
